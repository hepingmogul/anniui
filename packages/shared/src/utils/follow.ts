class Follow {
    ...
    **/
    constructor (eleTrigger: any, eleTarget: any, options: any) {
        // 默认参数值
        var defaults = {
            offsets: {
                x: 0,
        ...
        arrSafeArea = arrSafeArea.map(function (val: any) {
                    ...
                    if (isEdgeAdjust && objIsOverflow['top-bottom']) {
                        if (!objIsOverflow['bottom-top']) {
                            strPosition = ({
                                ...
                                '5-7': '7-5',
                                '2-3': '3-2'
                            } as Record<string, string>)[strPosition];
                            // 再执行一次
                            funGetPosition();
    ...
    zIndex () {
        ...
        // 此时元素的层级
        var numZIndexTarget = Number(objStyleTarget.zIndex);
        // 用来对比的层级，也是最小层级
        var numZIndexNew = 19;
        ...
        document.body.childNodes.forEach(function (eleChild) {
            ...
            var objStyleChild = window.getComputedStyle(eleChild as Element);

            var numZIndexChild = Number(objStyleChild.zIndex);
